...
def AttackingPositionOnField(details, event_data):
    ...
    event_data = event_data[~event_data['Action'].isin(['Tackle', 'Def Aerial', 'Progr Rec', 'Own Box Clear',
           ...
           'Unsucc Stand Tackle', 'Foul Lost', 'Unsucc Tackle', 'Unprogr Inter', 'Sub On', 'Sub Off', 'Kick Off'])]
    ...
    if team_name == 'Boston Bolts U13':
        dist_avg = pd.read_csv('PostMatchReviewApp_v2/ActionsAverages/AttackingActionsAverageU13.csv')
    elif (team_name == 'Boston Bolts U14') or (team_name == 'Boston Bolts U15'):
        dist_avg = pd.read_csv('PostMatchReviewApp_v2/ActionsAverages/AttackingActionsAverageU14U15.csv')
    elif (team_name == 'Boston Bolts U16') or (team_name == 'Boston Bolts U17') or (team_name == 'Boston Bolts U19'):
        dist_avg = pd.read_csv('PostMatchReviewApp_v2/ActionsAverages/AttackingActionsAverageU16U17U19.csv')
    ...
    for index, row in player_stats_xy.iterrows():
        ...
        ax.add_patch(circle)
        ax.text(row['x_mean'], row['y_mean'], row['Player Full Name'], color='black', size=8, ha='center', 
                             va='center', zorder=4)
