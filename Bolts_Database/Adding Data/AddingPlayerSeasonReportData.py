...
def load_processed_files():
    try:
        with open('processed_files_4.txt', 'r') as f:
...
def save_processed_files(processed_files):
    with open('processed_files_4.txt', 'w') as f:
        f.write('\n'.join(processed_files))

# Function to reset processed files
def reset_processed_files():
    if os.path.exists('processed_files_4.txt'):
        os.remove('processed_files_4.txt')
        logging.info("Reset processed_files_4.txt. All files will be reprocessed.")
...
def process_and_insert(file_path, connection):
    try:
        ...
        expected_columns = ['Player Full Name', 'mins played', 'Team Name', 'Yellow Card', 
        ...
        number_columns = ['mins played', 'Yellow Card', 'Red Card', 'Goal', 'Assist',
                      ...
                      'Success', 'Unsuccess', 'Pass Completion ', 'Progr Pass Completion ', 'Foul Won', 'Foul Conceded',
                      'Save Held', 'Save Parried', 'Save % ', 'Successful Cross', 'PK Missed', 'PK Scored']
        number_columns = [col for col in number_columns if col in df.columns]
        ...
        insert_query = f"""
        INSERT INTO player_non_position_season_report ({", ".join([f"`{col}`" for col in df.columns])})
        VALUES ({placeholders});
        """
