...
def load_processed_files():
    try:
        with open('processed_files_5.txt', 'r') as f:
...
def save_processed_files(processed_files):
    with open('processed_files_5.txt', 'w') as f:
        f.write('\n'.join(processed_files))

# Function to reset processed files
def reset_processed_files():
    if os.path.exists('processed_files_5.txt'):
        os.remove('processed_files_5.txt')
...
def process_and_insert(file_path, connection):
    try:
        ...
        insert_query = f"""
        INSERT INTO player_position_season_report ({", ".join([f"`{col}`" for col in df.columns])})
        VALUES ({placeholders});
        """
