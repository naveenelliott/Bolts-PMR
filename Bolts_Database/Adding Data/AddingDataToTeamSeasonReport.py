...
folder_path = 'IDP_Report/This_Year/'
processed_files = set()
...
def load_processed_files():
    try:
        with open('processed_files_6.txt', 'r') as f:
...
def save_processed_files(processed_files):
    with open('processed_files_6.txt', 'w') as f:
        f.write('\n'.join(processed_files))

# Function to reset processed files
def reset_processed_files():
    if os.path.exists('processed_files_6.txt'):
        os.remove('processed_files_6.txt')
...
def process_and_insert(file_path, connection):
    try:
        ...
        start_index = df.index[df["Period Name"] == "Running By Team"][0]
        df = df.iloc[start_index:].reset_index(drop=True)
        # Select a specific row (example: row at index 1)
        selected_row = df.iloc[1]

        # Convert the selected row to a DataFrame with a single column
        df = selected_row.to_frame().T  # Transpose to maintain row format
        
        df = df[['Team Name', 'mins played', 'Yellow Card', 
        ...
        insert_query = f"""
        INSERT INTO team_season_report ({", ".join([f"`{col}`" for col in df.columns])})
        VALUES ({placeholders});
        """
