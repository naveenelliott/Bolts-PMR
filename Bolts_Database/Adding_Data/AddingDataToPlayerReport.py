...
def addingDataToPlayerReport():
    ...
    def load_processed_files():
        try:
            with open('Bolts_Database/Run/Non_Position_Game.txt', 'r') as f:
    ...
    def save_processed_files(processed_files):
        with open('Bolts_Database/Run/Non_Position_Game.txt', 'w') as f:
            f.write('\n'.join(processed_files))

    # Function to reset processed files
    def reset_processed_files():
        if os.path.exists('Bolts_Database/Run/Non_Position_Game.txt'):
            os.remove('Bolts_Database/Run/Non_Position_Game.txt')
            logging.info("Reset Non_Position_Game.txt. All files will be reprocessed.")
    ...
    def process_and_insert(file_path, connection):
        try:
            ...
            expected_columns = ['Player Full Name', 'mins played', 'Match Date', 'Team Name', 'Yellow Card', 
                    ...
                    'Save Held', 'Save Parried', 'Save % ', 'Successful Cross', 'PK Missed', 'PK Scored', 'Opposition']
            ...
            insert_query = f"""
            INSERT INTO player_non_position_game_report ({", ".join([f"`{col}`" for col in df.columns])})
            VALUES ({placeholders});
            """
