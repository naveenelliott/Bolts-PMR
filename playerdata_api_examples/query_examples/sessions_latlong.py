...
goalkeepers = ['Aaron Choi', 'casey powers', 'Milo Ketnouvong', 'Benjamin Marro', 'Jack Seaborn', 
               'Jack Susi', 'Dylan Jacobson', 'Daniel Senichev', 'Sy Perkins', 'Torran Archer', 
               'Sawyer Emond', 'Liam muller', 'Drew Crosby', 'Devin Patel', 'Sawyer Jacobson', 'Kai Mercer', 
               'Douglas Mira', 'Jack Comes', 'Connor Rodday', 'Tiernan Forde', 'Neil Doherty', 'Ian Sarabia', 
               'Nolan McKenna', 'Jake McConville']

acceptable_pitches = ['UMass Mt. Ida', 'Mount Ida', 'Ida', 'UMass Mt Ida Turf', 'Umass MT. IDA']
...
def club_sessions(club_id):
    ...
    for row in result['club']['matchSessions']:
        ...
        pitch_name = row['pitch']['name']

        if pitch_name not in acceptable_pitches:
            continue
        ...
        for participation in session_participations:
            ...
            if athlete_name not in goalkeepers:
                continue  # Skip non-goalkeeper players
            ...
            for data in datafiles:
                ...
                try:
                    ...
                    df.to_csv(f"PlayerDataLatLong/{athlete_name}_{club_name}_{opp}_LatLong.csv", index=False)
...
if __name__ == "__main__":
    ...
    for team_name, team_code in team_codes.items():
        club_sessions(club_id=team_code)
